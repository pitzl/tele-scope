...
int main( int argc, char * argv[] )
{
  ...
  for( int i = 1; i < argc; ++i ) {
    ...
    if( !strcmp( argv[i], "-c" ) )
      forcedphcut = atoi( argv[++i] ) / 10. ; //arguments have to be integers
  ...
  */
  ...
  if( forcedphcut > 0 )
    rootFileName << "scopes" << run << "_th" << 10*forcedphcut << ".root";
  else
  ...
  for( list <evInfo>::iterator evinfo = infoList.begin(); evinfo != infoList.end() && iev < lev; ++evinfo ) {
	...
	//pixel px { col, row, ph, ph, 0 }; // ROC px
        ...
        pixel px { col, row, ph, ph, ph, 0, 999 }; // include noise
        vpx.push_back(px);
        dutadcAll.Fill( ph );
	++ipx;
	...
	//pixel px { col, vpx[ipx].row, vpx[ipx].adc, vpx[ipx].ph, vpx[ipx].q, 0 };
        pixel px { col, vpx[ipx].row, vpx[ipx].adc, vpx[ipx].ph, vpx[ipx].q, 0, 999 };
	...
	for( ; px4 != px7; ++px4 ) { // between 1 and 7, exclusively
	  ...
	  double sgmct = 4;
	    ...
	    if( pb.size() > 990 ) {
	      cout << "R4S pixel buffer overflow in event " << iev << endl;
	      break;
	    }
	  } // dph cut
	...
	colpxmap[col] = colpx;
        ...
        // fill biggest seed for central single pixel events
        if( abs( posseed ) > abs( negseed ) && posseed != 0 ){
          dutbigdph.Fill(  posseed );
        }
        else if ( negseed != 0 ) {
          dutbigdph.Fill( -negseed );
        }
  ...
  for( int aligniteration = DUTaligniteration; aligniteration < maxiter; ++aligniteration ) {
	...
	if( iev > 400 &&       // pedestal and noise
	  ...
	  for( int col = 0; col < 155; ++col ) { // x
	    ...
	    for( auto px = colpx[col].begin(); px != colpx[col].end(); ++px ) {
              ...
              pixel hitPix { px->col, px->row, px->adc, px->ph, px->q, px->big, 999 };
              pixel eptPix { -17, -17, 0, 0, 0, 0, 999}; // empty pixel for initialisation
          ...
          for( int col = 0; col < 155; ++col ) { // columns
            ...
            for( auto px = colpx[col].begin(); px != colpx[col].end(); ++px ) { // pixel
              ...
              pixel thisPx { px->col, px->row, px->adc, px->ph, px->q, px->big, 999 };
	...
	double clQ0 = 0;
        ...
        //cout << "  " << iev << " cluster size " << cl[iDUT].size() << endl;
	  ...
	  */
	  ...
	  if( fabs(x4) < 3.8 && fabs(y4) < 3.9 && // fiducial
	    ...
	    if( Q0 < 7.5 ) {
	      ...
	      cmsxyq0->Fill( x4, y4 ); // clusters size map
	    }
	    else
