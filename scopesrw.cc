...
// Daniel Pitzl, DESY, Sep 2017, Jan 2018: full run in-memory analysis (8 GB RAM)
// telescope analysis with eudaq and ROC4Sens and MOD

// for row-wise readout 2019
...
class pixelc {
 public:
  ...
  int ord;
  bool big;
  bool operator < (const pixelc & pxObj ) const // col-wise ordering
  {
...
struct cluster {
  vector <pixelc> vpix; // Armin Burgmeier: list
...
//------------------------------------------------------------------------------
vector < cluster > getClus( vector <pixelc> pb, int fCluCut = 1 ) // 1 = no gap
{
  ...
  while( seed < pb.size() ) {
    ...
    for( vector<pixelc>::iterator p = c.vpix.begin();  p != c.vpix.end();  ++p ) {
...
int main( int argc, char * argv[] )
{
  ...
  */
  ...
  /*
    effvsxy->Draw("contz")
    TArc c( -2.0, 1.0, 2.5 ); // circle
    c.SetFillStyle(0);
    c.SetLineWidth(3);
    c.Draw("same");
    TArc m( -2.0, 1.0, 0.1 ); // mid
    m.SetFillStyle(1000);
    m.Draw("same");
  */
  if( chip0 == 179 ) { // 36255 179i 40 mA 150V
    xbeam =-2.0;
    ybeam = 1.0;
    rbeam = 3.0;
  }
  /*
    effvsxy->Draw("contz")
    TArc c( -2.5, 1.0, 2.5 ); // circle
    c.SetFillStyle(0);
    c.SetLineWidth(3);
    c.Draw("same");
    TArc m( -2.5, 1.0, 0.1 ); // mid
    m.SetFillStyle(1000);
    m.Draw("same");
  */
  if( chip0 == 193 ) { // 36288 193i 40 mA 150V
    xbeam =-2.5;
    ybeam = 0.5;
    rbeam = 3.0;
  }
  ...
  double qxmax = 0.001; // = exp(-qmin/qwid) for qmin = 7.5 ke cutoff

  if( chip0 >= 119 && chip0 < 138 ) { // irrad gain_1
    qwid = 0.8;
    qxmax = 0.10; // irrad = 1.8 ke cutoff
  }
  ...
  if( !rot90 && run >= 36332 ) { // May 2019: cable from below, beam from back
    upsignx = 1;
    upsigny = 1;
  }
  if( !rot90 && run >= 36338 ) { // May 2019: cable from below, beam from front
    upsignx =-1;
    upsigny = 1;
  }
  ...
  if( chip0 == 176 && run >= 36332 ) ke = 0.0269; // 176 chilled at 11.3 ke
  if( chip0 == 176 && run >= 36339 ) ke = 0.0413; // 176 warm at 11.3 ke
  ...
  if( chip0 == 123 ) dphcut = 23; // irrad, gain_2, dp2 9
  ...
  if( chip0 == 200 ) dphcut = 15; // irrad, RMS 6 ADC
  if( chip0 == 201 ) dphcut = 20; // irrad, RMS 6 ADC
  if( chip0 == 207 ) dphcut = 12; // irrad, RMS 5 ADC
  if( chip0 == 176 ) dphcut = 10; // fresh, cool, dp2 3.4
  ...
  rootFileName << "scopesrw" << run << ".root";
  ...
  TH1I roicolsHisto( "roicols", "ROI columns;ROI columns;events", 156, -0.5, 155.5 );
  ...
  TProfile dutnpxvst2( "dutnpxvst2",
	      "DUT pixels vs time;time [s];DUT pixels per event / 10s",
	      200, 0, 2000, -0.5, 99.5 );
  TProfile dutnclvst2( "dutnclvst2",
	      "DUT clusters vs time;time [s];DUT clusters per event with pixels / 10s",
	      200, 0, 2000, -0.5, 99.5 );
  TProfile dutyldvst2( "dutyldvst2",
	      "DUT yield vs time;time [s];DUT events with pixels / 10s",
	      200, 0, 2000, -0.5, 1.5 );
  ...
  TH1I dutp0Histo( "dutp0",
		   "normal cluster PH;normal cluster pulse height [ADC];DUT clusters",
		   800, 0, 1600 );
  TH1I dutq0Histo( "dutq0",
		   "normal cluster charge;normal cluster charge [ke];DUT clusters",
		   480, 0, 120 );
  ...
  TProfile tridxvsx( "tridxvsx",
		     "triplet dx vs x;triplet xB [mm];<triplet #Deltax> [mm]",
		     240, -12, 12, -0.05, 0.05 );
  ...
  TProfile tridyvsx( "tridyvsx",
		     ...
		     240, -12, 12, -0.05, 0.05 );
  TProfile tridyvsy( "tridyvsy",
		     "triplet dy vs y;triplet yB [mm];<triplet #Deltay> [mm]",
		     120, -6, 6, -0.05, 0.05 );
  ...
  TH1I roiqHisto( "roiq", "ROI PH;ROI charge [ke];fiducial ROI", 340, -5, 80 );
  ...
  TH2I * cmsxxHisto = new
    TH2I( "cmsxx", "tracks vs DUT in x;track x [mm];DUT cluster x [mm];track-cluster pairs",
	  800, -8, 8, 800, -8, 8 );

  TH2I * cmsyyHisto = new
    TH2I( "cmsyy", "tracks vs DUT in y;track y [mm];DUT cluster y [mm];track-cluster pairs",
	  800, -8, 8, 800, -8, 8 );
  ...
  TProfile cmsmadxvsxm =
    TProfile( "cmsmadxvsxm",
	      "DUT x resolution vs xmod;x track mod 100 [#mum];MAD(#Deltax) [#mum]",
	      50, 0, 100, 0, 100 );
  TProfile cmsmadxvsq =
    TProfile( "cmsmadxvsq",
  ...
  TH1I effp0Histo =
    TH1I( "effp0",
	  "nearest cluster signal;DUT cluster signal [ADC];nearest cluster",
	  800, 0, 1600 );
  TH1I effp0cHisto =
    TH1I( "effp0c",
  ...
  TProfile2D * effp0vsxy = new
    TProfile2D( "effp0vsxy",
		"DUT cluster signal map;x track at DUT [mm];y track at DUT [mm];cluster peak signal [ADC]",
		90, -4.5, 4.5, 90, -4.5, 4.5 );
  ...
  TProfile effvsddt =
    TProfile( "effvsddt",
	      "DUT efficiency vs ddt;DBT-TLU #Deltadt time [us];efficiency",
	      500, -500, 500, -1, 2 );
  ...
  list < map < int, set <pixelc> > > pxlist; // for ROI
  ...
  do {
    ...
    if( iev < 10 )
      cout << "scopes reading  " << run << "." << iev << "  taken " << evsec << endl;
    else if( iev < 100 && iev%10 == 0 )
      cout << "scopes reading  " << run << "." << iev << "  taken " << evsec << endl;
    else if( iev < 1000 && iev%100 == 0 )
      cout << "scopes reading  " << run << "." << iev << "  taken " << evsec << endl;
    else if( iev%1000 == 0 )
      cout << "scopes reading  " << run << "." << iev << "  taken " << evsec
	   << " dt " << tludt << endl;
	...
	ipl -= 1; // 0..5
      ...
      vector <pixelc> pb; // for clustering
  ...
  string hd;
  bool row_wise = 0;
  string ROW_WISE {"row_wise"};
  while( hd != START ) {
    getline( evFile, hd ); // read one line into string
    cout << "  " << hd << endl;
    stringstream tokenizer( hd );
    string tag;
    tokenizer >> tag; // leading white space is suppressed
    if( tag == ROW_WISE ) 
      tokenizer >> row_wise;
  ...
  if( !row_wise ) {
    cout << "scopesrw only for row-wise readout" << endl;
    return 3;
  }
  ...
  for( list <evInfo>::iterator evinfo = infoList.begin(); evinfo != infoList.end() && iev < lev; ++evinfo ) {
    ...
    istringstream DUTiss( DUTev ); // tokenize string

    // 198 E 200 86613583701
    // 203 F 400 86667558753

    int dut_ev;
    DUTiss >> dut_ev; // DUT event
    ...
    if( filled == F )
      DUTiss >> dtbtime; // from run 456 = 31093
	...
	break;
      ...
      break; // only once (no effect in 31163)
    ...
    map < int, set <pixelc> > rowpxmap; // per row, sorted along col
	...
	hcol[iDUT].Fill( col+0.5 ); // ROI
	hrow[iDUT].Fill( row+0.5 );

	pixelc px { col, row, ph, ph, 0, 0 }; // ROC px
	vpx.push_back(px);
	++ipx;
      ...
      roiszHisto.Fill( vpx.size() );
      if( vpx.size() )
	roiszlHisto.Fill( log( vpx.size() ) / log10 );
	...
	filled = A;
      ...
      set <pixelc> compxr[160]; // per row, sorted along col

      for( unsigned ipx = 0; ipx < vpx.size(); ++ipx ) {
	int row = vpx[ipx].row;
	pixelc pxc { vpx[ipx].col, row, vpx[ipx].ph, vpx[ipx].q, 0, 0 }; // ROC px
	compxr[row].insert(pxc); // sorted along col
      }

      for( unsigned row = 0; row < 160; ++row ) {

	if( compxr[row].size() < 2 ) continue;

	set <pixelc> rowpx; // ROC px per row, sorted along col

	auto px1 = compxr[row].begin();
	auto px7 = compxr[row].end(); --px7; // last col in ROI
	...
	for( ; px4 != px7; ++px4 ) { // between 1 and 7, exclusively
	  ...
	  rowpx.insert(px); // sorted along col
	...
	rowpxmap[row] = rowpx; // all ROI pix
    ...
    roicolsHisto.Fill( rowpxmap.size() ); // peaks at 7, 14, 21, 28

    if( iev > rev ) rowpxmap.clear(); // 7 GB memory

    pxlist.push_back( rowpxmap ); // for ROI
    ...
    for( vector<cluster>::iterator c = vcl.begin(); c != vcl.end(); ++c ) {
      ...
      for( vector<pixelc>::iterator px = c->vpix.begin(); px != c->vpix.end(); ++px ) {
  ...
  for( int aligniteration = DUTaligniteration; aligniteration < maxiter; ++aligniteration ) {
	...
	cl[ipl] = *evi.at(ipl);
      ...
      auto rowpx = *pxit; // ROI: col-map of sets of row-px
	...
	tlutime0 = tlutime;
      ...
      bool lddt = 1;
      if( fabs( tludt - dtbdt ) > 20E-6 ) lddt = 0;
      /*
      cout << iev
	   << "  " << tlutime << "  " << tludt
	   << "  " << dtbtime << "  " << dtbdt
	...
	for( vector<cluster>::iterator cC = cl[2].begin(); cC != cl[2].end(); ++cC ) {
      ...
      if( run == 31152 ) { // cmsdxvsev->Fit("pol1")

	double p0 = -0.00115974;
	double p1 = 8.09957e-09;
	...
	DUTaligny = DUTaligny0 + 0.008 - 4.555e-9*iev; // trend correction
      ...
      if( run == 31315 ) { // cmsdxvsev->Fit("pol4")
	...
	if( ttdmin > 0.2 ) liso = 1; // more data
	...
	for( unsigned int jB = 0; jB < driplets.size(); ++jB ) { // j = B = downstream
	  ...
	  if( fabs(dx) < sixcut && fabs(dy) < sixcut ) {
	    ...
	    xmod = fmod( 9.000 + x4, 0.100 ); // [0,0.1] mm
	    ymod = fmod( 9.000 + y4, 0.100 ); // [0,0.1] mm
	    if( chip0 == 142 || chip0 == 143 || chip0 == 144 )
	      ymod = fmod( 9.050 + y4, 0.100 ); // [0,0.1] mm
	    xmod2 = fmod( 9.000 + x4, 0.200 ); // [0,0.2] mm
	    ymod2 = fmod( 9.000 + y4, 0.200 ); // [0,0.2] mm
	    xmod5 = fmod( 9.000 + x4, 0.050 );
	    ymod5 = fmod( 9.000 + y4, 0.050 );
	...
	if( iev > 400 &&       // pedestal and noise
	  ...
	  for( int row = 0; row < 160; ++row ) { // y
	    ...
	    if( rowpx.count(row) < 1 ) continue; // empty
	...
	for( vector<cluster>::iterator c = cl[iDUT].begin(); c != cl[iDUT].end(); ++c ) {
	  ...
	  if( chip0 == 193 ) // noisy irrad
	    isoc = 1;
	  if( chip0 == 197 ) // noisy irrad ?
	    isoc = 1;
	  if( chip0 == 200 ) // noisy irrad ?
	    isoc = 1;
	  if( chip0 == 207 ) // noisy irrad ?
	    isoc = 1;
	  ...
	  if( run >= 31632 &&
	      chip0 >= 119 && chip0 <= 138 ) { // Feb-Mar 2018 irrad gain_2
	    lq = 1;
	    if( ( Q0 < 3 || Q0 > 12 ) ) lq = 0;
	  }
	  ...
	  if( run >= 34406 && // Nov 2018: 8E15 n/cm2, peak 5.8 ke
	  ...
	  cmsxxHisto->Fill( x4, cmsx );
	  cmsyyHisto->Fill( y4, cmsy );
	...
	if( lsixlk
	  ...
	  //if( dddmin > 0.3 && ttdmin > 1.4 ) { // stronger iso [mm] 99.94
	  //if( dddmin > 0.3 && ttdmin > 0.6 ) { // iso [mm] 99.93
	  if( iev > 400 &&
	    ...
	    if( fiducial ) {
	      ...
	      effvsddt.Fill( (tludt-dtbdt)*1E6, max( 0.001, 1.0*nm[49] ) );
		...
		if( nm[49] ) {
		  effq0Histo.Fill( clQ0 ); // 25.11.2018
	...
	delete fdxvsy;
      ...
      if( fabs(DUTturn) > 0.3 && cmsdxvsx.GetEntries() > 999 ) {
	...
	delete fdyvsx;
      ...
      if( fabs(DUTtilt) > 0.3 && cmsdyvsy.GetEntries() > 999 ) {
